...
| File | Description |
|------|-------------|
| `deploy.sh` | Deploy script: ingress, then the job phases, then the stack. The full stack file is deployed with `--prune`, so services removed from the spec are removed from the swarm on the next deploy, and with `--detach=false`, so the script only returns once every task is running (and healthy, where a `healthcheck` is declared). Unused images are pruned afterwards |
