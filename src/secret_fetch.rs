...
impl FetchScript {
    ...
    pub fn write_to_file(&mut self, secret: &SecretResolvedSpec, path: &str) {
        ...
        self.lines
            .push(format!("printf '%s' \"${}\" > {}", secret.shell_var(), sh_quote(&path)));
        ...
        self.lines
            .push(format!("chmod {} {}", SECRET_FILE_MODE, sh_quote(&path)));
    ...
    pub fn write(&self, path: &Path, usage: &str) -> Result<()> {
        ...
        writeln!(
            file,
            "# Generated by simpled. Reads the secrets declared with an `aws:` source from"
        )?;
        writeln!(
            file,
            "# AWS Secrets Manager, here on the deploy target, so their values never travel"
        )?;
        ...
        writeln!(
            file,
            "# Credentials, region and profile are taken from the AWS CLI's own environment"
        )?;
        ...
        writeln!(
            file,
            "# Create the files below readable by their owner only. Each is widened to"
        )?;
        writeln!(
            file,
            "# {} right after it is written, so the value is never in a file more readable",
            SECRET_FILE_MODE
        )?;
        ...
        writeln!(
            file,
            "# Read one secret and print its value. Fails the deploy when the secret does"
        )?;
        writeln!(
            file,
            "# not exist, cannot be read, holds no string value, or — with a jq filter —"
        )?;
        writeln!(
            file,
            "# has no such key, rather than letting a service start on a blank credential."
        )?;
        ...
        writeln!(
            file,
            "  # --query prints the literal None for a secret that holds only binary data,"
        )?;
        writeln!(file, "  # and nothing at all for an empty string value.")?;
        writeln!(file, "  if [ -z \"$value\" ] || [ \"$value\" = 'None' ]; then")?;
        writeln!(
            file,
            "    echo \"Secret $name: '$secret_id' has no string value in AWS Secrets Manager.\" >&2"
        )?;
        writeln!(file, "    return 1")?;
        writeln!(file, "  fi")?;
        ...
        writeln!(
            file,
            "    # -r keeps a JSON string unquoted, -e turns the null that a missing key"
        )?;
        writeln!(
            file,
            "    # selects into a failure. jq's own stderr is dropped: a parse error quotes"
        )?;
        writeln!(file, "    # the input it choked on, which is the secret.")?;
        writeln!(
            file,
            "    if filtered=\"$(printf '%s' \"$value\" | jq -er \"$filter\" 2>/dev/null)\"; then"
        )?;
        ...
        writeln!(
            file,
            "    echo \"Secret $name: '$secret_id' resolved to an empty value.\" >&2"
        )?;
        writeln!(file, "    return 1")?;
        writeln!(file, "  fi")?;
        ...
        if self.gives_back {
            writeln!(
                file,
                "# Run with sudo, everything below would belong to root, and the user that"
            )?;
            writeln!(
                file,
                "# copies the next deployment in could no longer overwrite it. Give each path"
            )?;
