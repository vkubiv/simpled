...
fn parse_service_volume(s: &str) -> Result<ServiceVolume> {
    let (vol_str, mount_path) = s.split_once(':').ok_or_else(|| {
        anyhow!(
            "Invalid volume format '{}'. Expected 'name:mount_path' or './path:mount_path'",
            s
        )
    })?;
