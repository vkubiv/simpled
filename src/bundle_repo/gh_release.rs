...
pub fn download(repo: &str, ver: &str, app_name: &str, tag_prefix: Option<&str>) -> Result<String> {
    ...
    // Check GITHUB_TOKEN
    let token =
        env::var("GITHUB_TOKEN").context("GITHUB_TOKEN is not set. It is required for downloading releases.")?;
    ...
    if !response.status().is_success() {
        bail!(
            "Failed to get release info from {}: status code {}",
            url,
            response.status()
        );
    }
    ...
    if !response.status().is_success() {
        bail!(
            "Failed to download asset from {}: status code {}",
            asset.url,
            response.status()
        );
    }
