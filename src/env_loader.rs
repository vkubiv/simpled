...
pub fn parse_env_variable(input: &str) -> Result<EnvVariable> {
    ...
    Ok(EnvVariable {
        ...
        value: desc
            .default
            .ok_or_else(|| anyhow!("No value for env variable: {}", input))?,
    })
}
...
pub fn load_env_file<P: AsRef<Path>>(path: P) -> Result<Vec<EnvVariable>> {
    ...
    for (line_num, line) in reader.lines().enumerate() {
        ...
        let parsed = parse_env_variable(trimmed)
            .with_context(|| format!("Failed to parse env variable on line {}: {}", line_num + 1, trimmed))?;
...
mod tests {
    ...
    fn test_parse_env_string() {
        ...
        assert_eq!(
            ...
            EnvDescriptor {
                name: "FOO".to_string(),
                default: Some("bar=bar".to_string())
            }
        );
